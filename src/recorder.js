...
(function(global) {
  ...
  function gameActive() {
    try {
      return ["round", "tiebreak", "battle", "rackrush", "wincine", "victorycine", "replay"].includes(G.state) || state.capturing;
    } catch (e) {
  ...
  function drawCover(ctx, img, x, y, w, h, mirror) {
    ...
    const s = Math.max(w / sw, h / sh),
      ...
      dy = (h - dh) * .5;
    ctx.save();
    if (mirror) { ctx.translate(x + w, y);
      ctx.scale(-1, 1);
      ctx.drawImage(img, dx, dy, dw, dh); } else ctx.drawImage(img, x + dx, y + dy, dw, dh);
    ctx.restore();
  }
  ...
  function metricScore(record) {
    ...
    const attempts = Number(record && record.attempts) || 0,
      makes = Number(record && record.makes) || 0;
  ...
  function tierForScore(score) {
    if (score >= 92) return { accent: "#ffd700", dim: "#9f8300", soft: "rgba(255,215,0,.22)", text: "#fff0a8", stamp: "LEGENDARY" };
    if (score >= 84) return { accent: "#d9e1e7", dim: "#7e8b92", soft: "rgba(217,225,231,.18)", text: "#f2f6f8", stamp: "SILVER / ELITE" };
    if (score >= 74) return { accent: "#c78945", dim: "#7b4e24", soft: "rgba(199,137,69,.2)", text: "#ffd9aa", stamp: "BRONZE / STEADY" };
    if (score >= 64) return { accent: "#b96f38", dim: "#61351a", soft: "rgba(185,111,56,.18)", text: "#ffc49a", stamp: "COPPER / HOT HAND" };
    if (score >= 52) return { accent: "#7fd6e8", dim: "#336575", soft: "rgba(127,214,232,.16)", text: "#d6f7ff", stamp: "STEEL / ROOKIE" };
    if (score >= 38) return { accent: "#8e969d", dim: "#41474c", soft: "rgba(142,150,157,.17)", text: "#d9dddf", stamp: "SLATE / PRACTICE" };
    return { accent: "#666b70", dim: "#2d3034", soft: "rgba(120,126,132,.13)", text: "#c7cacc", stamp: "GRAY ZONE" };
  }

  function cardFromRecord(record, opts) {
    opts = opts || {};
    record = record || {};
    const battle = record.mode === "percent-battle",
      speed = record.variant === "speed100" || record.mode === "rack-rush-speed100";
    const dna = metricScore(record),
      tier = tierForScore(dna);
  ...
  function fitText(ctx, text, x, y, max, size, min) {
    ...
    while (s > min && ctx.measureText(text).width > max) { s -= 2;
      ctx.font = "900 " + s + "px Orbitron, monospace"; }
  ...
  function drawResultCard(ctx) {
    ...
    const fadeIn = Math.min(1, age / 650),
      fadeOut = age > RESULT_HOLD_MS ? Math.max(0, 1 - (age - RESULT_HOLD_MS) / 900) : 1,
      alpha = fadeIn * fadeOut;
    const c = state.resultCard,
      t = c.tier,
      x = 54,
      y = 400,
      w = W - 108,
    ...
    grd.addColorStop(0, t.soft);
    grd.addColorStop(.42, "rgba(18,20,20,.96)");
    grd.addColorStop(1, "rgba(11,12,12,.98)");
    ctx.fillStyle = grd;
    ...
    [
      [x + 22, y + 22, 28, 0, 0, 28],
      [x + w - 50, y + 22, 28, 0, 28, 28],
      [x + 22, y + h - 50, 28, 28, 0, 28],
      [x + w - 50, y + h - 50, 28, 28, 28, 28]
    ].forEach(a => { ctx.beginPath();
      ctx.moveTo(a[0], a[1] + a[5]);
      ctx.lineTo(a[0], a[1]);
      ctx.lineTo(a[0] + a[4], a[1]);
      ctx.stroke(); });
    ctx.fillStyle = t.accent;
    ctx.font = "900 18px Orbitron, monospace";
    ctx.fillText(c.mode, x + 32, y + 52);
    ctx.fillStyle = "#fff";
    fitText(ctx, c.title, x + 32, y + 112, w - 64, 40, 24);
    ctx.save();
    ctx.translate(x + 54, y + 145);
    ctx.rotate(0.16);
    ctx.strokeStyle = t.accent;
    ctx.lineWidth = 5;
    ctx.fillStyle = "rgba(18,20,20,.58)";
    ctx.fillRect(0, 0, w - 108, 52);
    ctx.strokeRect(0, 0, w - 108, 52);
    ctx.fillStyle = t.accent;
    fitText(ctx, t.stamp, 18, 36, w - 140, 30, 18);
    ctx.restore();
    ctx.fillStyle = t.accent;
    ctx.font = "900 74px Orbitron, monospace";
    ctx.fillText(c.score, x + 32, y + 206);
    ...
    ctx.fillText(c.sub, x + 30, y + 230);
    ctx.fillStyle = "rgba(255,255,255,.08)";
    roundRect(ctx, x + 28, y + 258, w - 56, 72, 10);
    ctx.fill();
    ctx.fillStyle = "#dce8f4";
    ctx.font = "800 17px Orbitron, monospace";
    ctx.fillText(c.statA, x + 48, y + 287);
    ctx.fillStyle = t.accent;
    ctx.fillText(c.statB, x + 48, y + 317);
    const rt = rankText();
    ctx.fillStyle = rt ? t.accent : "#9ab2c5";
    ctx.font = "900 22px Orbitron, monospace";
    ctx.fillText(rt || "GLOBAL RANK PENDING", x + 30, y + 360);
    ctx.restore();
  }

  function drawHud(ctx) {
    ctx.save();
    const grd = ctx.createLinearGradient(0, 0, 0, 260);
    grd.addColorStop(0, "rgba(3,6,14,.72)");
    grd.addColorStop(1, "rgba(3,6,14,0)");
    ctx.fillStyle = grd;
    ...
    ctx.fillText(gameLabel(), 34, 154);
    ctx.fillStyle = "rgba(255,255,255,.65)";
    ctx.font = "700 16px Orbitron, monospace";
    ctx.fillText(state.lastLabel || "LAST SHOT", 34, 184);
    ctx.fillStyle = "rgba(255,255,255,.82)";
    ctx.font = "700 15px Orbitron, monospace";
    ctx.fillText(playerText(), 34, 210);
    const rt = rankText();
    if (rt) { ctx.fillStyle = "#7CFC6B";
      ctx.font = "900 17px Orbitron, monospace";
      ctx.fillText(rt, 34, 236); }
    ctx.fillStyle = "rgba(0,0,0,.45)";
    ctx.fillRect(0, H - 136, W, 136);
    ctx.fillStyle = "#fff";
  ...
  function drawVisionPip(ctx) {
    ...
    const vw = (frame && frame.width) || v.videoWidth,
      vh = (frame && frame.height) || v.videoHeight,
      portrait = frame ? !!frame.portrait : vh > vw;
    const displayAspect = (frame && frame.displayAspect) || (vw / vh);
    let w = portrait ? (MOBILE ? 142 : 170) : 196,
      h = Math.round(w / displayAspect),
      maxH = portrait ? (MOBILE ? 254 : 304) : 160;
    if (h > maxH) { w = Math.round(w * maxH / h);
      h = maxH; }
    const resultVisible = !!state.resultCard,
      x = resultVisible ? W - w - 30 : 30,
      y = resultVisible ? 68 : H - 154 - h;
    ctx.save();
    ctx.fillStyle = "rgba(0,0,0,.62)";
    roundRect(ctx, x - 7, y - 28, w + 14, h + 35, 8);
    ctx.fill();
    ...
    ctx.fillText("LOCAL POSE", x, y - 9);
    ctx.beginPath();
    roundRect(ctx, x, y, w, h, 6);
    ctx.clip();
    ctx.fillStyle = "#050912";
    ctx.fillRect(x, y, w, h);
    drawContain(ctx, v, x, y, w, h, true);
    const overlay = document.getElementById("visionCanvas");
    if (overlay && overlay.width) { ctx.globalAlpha = .78;
      drawContain(ctx, overlay, x, y, w, h, true);
      ctx.globalAlpha = 1; }
    ctx.restore();
  }
  ...
  function draw(ctxObj) {
    const c = canvas(),
      ctx = state.ctx,
      source = (ctxObj && ctxObj.canvas) || document.getElementById("c");
    if (!source) return;
    ...
    ctx.restore();
    ctx.fillStyle = "rgba(3,6,12,.2)";
    ctx.fillRect(0, 0, W, H);
    for (let y = 0; y < H; y += 6) { ctx.fillStyle = "rgba(255,255,255,.025)";
  ...
  function updateStatus(txt) {
    ...
    if (el) el.textContent = txt || statusText();
    const btn = document.getElementById("clipSaveBtn");
    if (btn) btn.disabled = !supported();
  }

  function statusText() {
    if (!supported()) return "Recording not supported in this browser";
    if (state.capturing) return state.armed ? "Pre‑recording last round…" : "Generating highlight…";
    if (state.lastBlob) return /mp4/i.test(state.lastBlob.type) ? "MP4 highlight ready" : "Highlight ready (WebM)";
    return "Auto‑generate after each clutch shot";
  }
  ...
  function startRecording(label, opts) {
    opts = opts || {};
    state.lastLabel = label || state.lastLabel || "highlight";
    state.lastBlob = null;
    ...
    const mt = mimeType(),
      optsRec = { videoBitsPerSecond: VIDEO_BPS, audioBitsPerSecond: MOBILE ? 128000 : 192000 };
    if (mt) optsRec.mimeType = mt;
    state.rec = new MediaRecorder(state.stream, optsRec);
    state.rec.ondataavailable = onData;
    state.rec.onstop = finalizeClip;
    state.rec.onerror = () => { state.capturing = false;
      state.armed = false;
      updateStatus("Highlight generation failed"); };
    state.rec.start(250);
    state.capturing = true;
    state.armed = !!opts.armed;
    state.startedAt = performance.now();
    const fmt = /mp4/i.test(mt) ? "MP4" : "WebM";
    updateStatus(state.audioTracks.length ? `Generating ${fmt}${state.armed ? " (pre‑recording)" : ""} with live audio` : `Generating ${fmt}${state.armed ? " (pre‑recording)" : ""} without audio`);
  }

  function arm(label) {
    if (!supported()) return false;
    if (state.capturing) return true;
    try { startRecording(label || "pre‑record last round", { armed: true }); return true; } catch (e) { state.capturing = false;
      state.armed = false;
      updateStatus("Pre‑record failed"); return false; }
  }

  function scheduleStop(ms, minHold, capToTarget) {
    if (!state.capturing) return false;
    const now = performance.now(),
      age = Math.max(0, now - state.startedAt),
      minimum = Math.max(0, minHold || 0);
    ...
    clearTimeout(state.stopTimer);
    state.stopAt = now + delay;
    state.stopTimer = setTimeout(stopRecording, delay);
    return true;
  }
  ...
  function mark(label, opts) {
    if (!supported()) return false;
    opts = opts || {};
    state.lastLabel = label || "highlight";
    state.lastBlob = null;
    if (state.capturing) { state.armed = false;
      scheduleStop(opts.postMs || POST_MS, 0);
      updateStatus("Last three shots captured – continuing celebration..."); return true; }
    try {
      startRecording(state.lastLabel, { armed: false });
      scheduleStop(opts.postMs || POST_MS, 0);
      return true;
    } catch (e) { state.capturing = false;
      state.armed = false;
      updateStatus("Highlight generation failed"); return false; }
  }

  function extend(ms) {
    if (!state.capturing) return false;
    scheduleStop(ms || POST_MS, 0);
    updateStatus("Recording celebration and scorecard...");
    return true;
  }
  ...
  function result(record, opts) {
    ...
    updateStatus("Recording celebration, scorecard, and global rank...");
    return true;
  }
  ...
  function rankUpdated() {
    ...
    const now = performance.now(),
      remaining = Math.max(0, state.stopAt - now);
  ...
  function finalizeClip() {
    ...
    if (!parts.length) { updateStatus("No clip to save"); return; }
    ...
    state.stopAt = 0;
    updateStatus(statusText());
    try { if (typeof toast === "function") toast(/mp4/i.test(state.lastBlob.type) ? "MP4 highlight ready" : "Highlight ready (WebM)", "#7CFC6B"); } catch (e) {}
    if (state.saveWhenReady) { state.saveWhenReady = false;
      save(); }
  }

  function filename() {
    let seed = "highlight";
    try { seed = GAME_SEED || seed; } catch (e) {}
    const ext = state.lastBlob && /mp4/i.test(state.lastBlob.type) ? "mp4" : "webm";
    return "aiba-" + seed + "-" + Date.now() + "." + ext;
  }

  function save() {
    if (!supported()) { try { toast("Recording not supported", "#ff8d7a"); } catch (e) {} return false; }
    if (!state.lastBlob) { state.saveWhenReady = true;
      updateStatus("Auto‑save pending…");
      try { toast("Highlight still generating, please wait", "#ffd23f"); } catch (e) {} return false; }
    if (typeof global.playSFX === "function") global.playSFX("ui_save_video_01");
    const a = document.createElement("a");
    a.href = state.lastUrl || URL.createObjectURL(state.lastBlob);
    a.download = filename();
    document.body.appendChild(a);
    a.click();
    a.remove();
    try { toast("Highlight saved", "#7CFC6B"); } catch (e) {}
    return true;
  }

  function resultMarkup() {
    if (!supported()) return "";
    return `<div class="clipExport"><button id="clipSaveBtn" class="btn sm" onclick="AIBARecorder.save()">🎞 Save MP4</button><small id="clipStatus">${wantsMp4() ? statusText() : "MP4 not supported – will fallback to WebM"}</small></div>`;
  }

  global.AIBARecorder = Object.freeze({
    tick,
    arm,
    mark,
    result,
