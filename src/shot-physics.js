...
(function(global){
  "use strict";
  const S={t:0,lastCharging:false,apexed:false,late:0,releaseLate:0,airborne:false,justLanded:false};
  ...
  function reset(){
    S.t=0;S.lastCharging=false;S.apexed=false;S.late=0;S.releaseLate=0;S.airborne=false;S.justLanded=false;
  }
  function update(opts){
    ...
    S.justLanded=false;
    if(charging){
      if(!S.lastCharging){S.t=0;S.releaseLate=0;}
      if(!paused)S.t+=dt;
    }
    else{
      if(S.lastCharging){S.airborne=S.t>=takeoff;S.releaseLate=S.late;}
      if(S.airborne){
        S.t+=dt;
    ...
    const fallSpan=Math.max(.12,p.land-p.peak),afterPeak=S.t>p.peak,fall=ease((S.t-p.peak)/fallSpan);
    ...
    const jump=resting?0:(afterPeak?Math.max(0,Math.max(base.jmp||0,1)*(1-fall)):Math.max(base.jmp||0,rise));
    const curve={
    ...
    return {
      curve,jump,late,apexCue,
      autoRelease:charging&&!paused&&S.t>=p.auto,
      airborne:S.airborne,
      justLanded:S.justLanded,
      t:S.t,
      params:p
    };
  }
