(function(global){
  "use strict";
  const T=s=>(global.AIBAI18N?global.AIBAI18N.t(s):s);
  ...
  function fallbackSave(blob,opts){
    ...
    if(opts.toast)opts.toast(opts.savedText||T("战报已保存 · 发到朋友圈吧!"),"#7CFC6B");
  }
  ...
  function genPoster(champ,ctx){
    ...
    c.fillText(champ?T("夺冠了!"):T("挑战结束"),W/2,216);
    ...
    if(G.finalist){
      ...
      c.fillText((champ?T("击败"):T("不敌"))+" "+T(G.finalist.n)+" (#"+G.finalist.num+")",W/2,380);
    }

    const stats=[
      [T("最高连中"),"x"+((G.stats&&G.stats.best)||0)],
      [T("花球命中"),((G.stats&&G.stats.moneyM)||0)+"/"+((G.stats&&G.stats.moneyT)||0)],
      [T("深远三分"),((G.stats&&G.stats.deepM)||0)+"/"+((G.stats&&G.stats.deepT)||0)],
      [T("难度"),T((DIFFS[G.diff]&&DIFFS[G.diff].n)||G.diff||"-")]
    ];
    ...
    const names=(G.opponents||[]).map(o=>T(o.n)+"#"+o.num).join(" · ");
    c.fillText(T("对阵:")+names,W/2,692);
    ...
    c.fillText(T("本场称号 · 「")+T(title)+T("」"),W/2,762);
    c.font="13px Orbitron, sans-serif";c.fillStyle="#d9f6ff";
    c.fillText(T("下一场,换你接管最后一投。"),W/2,784);
    c.font="bold 16px Orbitron, sans-serif";c.fillStyle="#dde";
    c.fillText(T("你能打败库里吗? 打开链接挑战同题 →"),W/2,850);
      ...
      if(navigator.share&&navigator.canShare){
        ...
        if(navigator.canShare({files:[file]})){
          navigator.share({files:[file],title:GAME_NAME+" #"+GAME_SEED,
            text:(champ?T("我夺冠了!")+" ":"")+sc+T("分 · 拿下「")+T(title)+T("」称号,你能超过我吗?")
