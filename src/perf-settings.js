/* ---------------- perf settings: player‑tunable performance options + live FPS ----------------
   Purpose: let players (especially iOS) toggle performance options on real devices with live FPS/draw call feedback.
   All changes apply instantly and are saved to localStorage. Loaded after the main script, it accesses
   global lexical scope for RENDER_QUALITY, renderer, scene, firePts, etc.
   Note: On iOS, "Chrome" is actually WebKit — most lag comes from fill rate / draw calls, and these toggles target exactly that. */

(function(global){
  "use strict";

  // Force English as the default language for the settings panel
  try {
    if (!localStorage.getItem("aiba-lang")) {
      localStorage.setItem("aiba-lang", "en");
    }
  } catch(e) {}
  ...
  /* ---------- Resolution floor: relax adaptive downscaling for battery saver ---------- */
  ...
  /* ---------- Transparent light cones: scan indoorRoot for ConeGeometry ---------- */
  function coneMeshes(){
    const out=[];
  ...
  /* ---------- Crowd density: Full / Light (reuses perf's 2/5) / Off ---------- */
  function nearPeople(){
  ...
  /* ---------- Fire trails / confetti particles ----------
     firePts is always visible (controlled by position lifecycle), so we can toggle its visibility directly.
     confPts is hidden by default and triggered by startConfetti, so when hidden we force it false,
     and when unhidden we don't force it on — the event system controls it. */
  ...
  /* ---------- Auto‑downscale on mobile: only down, never up during a round; frozen while recording ---------- */
  function autoActive(){
  ...
  function setAutoTier(next){
    ...
    try{
      if(typeof toast==="function")toast(autoTier===1?"Phone smooth mode enabled":"Phone performance guard boosted",autoTier===1?"#7ee7ff":"#ffd23f");
    }catch(e){}
  }
  ...
  // When particles are hidden, prevent confetti from re‑appearing
  ...
  /* ---------- Live FPS / draw call / resolution meter ---------- */
  ...
  function crowdRow(){
    const opts=[["full","Full"],["thin","Light"],["off","Off"]];
    return `<div class="perfRow perfSeg"><span class="perfRowMain"><b>Crowd density</b><em>Nearby crowd is the draw‑call hog; off saves the most</em></span>
  ...
  function identityRow(){
    ...
    return `<button class="perfIdentity" type="button" onclick="showNicknameEditor('settings')"><span><small>PLAYER</small><b>${esc(name||"No nickname yet")}</b></span><em>${name?"Edit nickname":"Set nickname"} ›</em></button>`;
  }
  function langRow(){
    const cur=(global.AIBAI18N&&global.AIBAI18N.lang)||"zh";
    const opts=[["zh","中文"],["en","English"]];
    return `<div class="perfRow perfSeg"><span class="perfRowMain"><b>Language</b><em>Switching reloads the page</em></span>
  ...
  function panelMarkup(){
    return `<div class="perfPanel">
      <div class="perfHead"><small>GAME SETTINGS</small><h1>Game settings</h1><p>Tune your nickname and performance here.</p></div>
      ${identityRow()}
      ${langRow()}
      <button class="perfHelp" type="button" data-aiba-icon="book-open" data-aiba-label="Help & guides" onclick="window.AIBAOnboard&&AIBAOnboard.help('settings')">Help & guides</button>
      ${toggleRow("autoTune","Auto smoothness guard",AUTO_DEVICE?(autoTier?`Auto‑lowered to tier ${autoTier} — stays put while recording`:"Trims crowd & FX automatically on sustained low FPS"):"Active on phones; desktop keeps full quality")}
      ${toggleRow("lowRes","Battery‑saver resolution","Lowers the render‑resolution floor — the most direct FPS boost (slightly softer image)")}
      ${toggleRow("hideCones","Disable light cones","Turns off arena light shafts — a big iOS fill‑rate cost")}
      ${crowdRow()}
      ${toggleRow("hideParticles","Disable fire/confetti FX","Turns off hot‑hand flames and celebration confetti")}
      ${toggleRow("fpsMeter","Show FPS meter","Always show FPS / draw calls / resolution scale (top right)")}
      <div class="perfActions">
        <button class="btn sm" type="button" onclick="AIBAPerfReset()">Reset defaults</button>
        <button class="btn" type="button" onclick="AIBAPerfClose()">Done</button>
      </div>
    </div>`;
  }
  ...
  function close(){
    ...
    if(playing&&typeof global.hidePanel==="function")global.hidePanel();
  ...
  function mountButton(){
    ...
    b.id="perfBtn";b.type="button";b.title="Game settings";b.textContent="Game settings";
    if(global.AIBASetIcon)global.AIBASetIcon(b,"settings","Game settings");
