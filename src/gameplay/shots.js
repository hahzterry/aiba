...
function updPowerUI() {
  const s = curShot();
  const isDeep = s && (s.deep != null || s.super);
  ...
  if (hide) {
    ...
    if (!G.blindToasted) {
      G.blindToasted = true;
      let msg = "Blind mode! Meter hidden · shoot on rhythm";
      if (G.diff === "easy") msg = "Rookie challenge upgrade · last 30% hides the shot meter";
      else if (G.mode === "battle") msg = "First 5 balls warm-up over · meter hidden, shoot by feel";
      else if (G.mode === "rackrush") msg = "This stage enters feel-shooting · meter hidden";
      toast(msg, "#ffd23f");
    }
    return;
  }
  const ideal = weatherAdjustedIdeal(s, false),
    zone = playerSweetZone();
  const z = $("pZone"),
    sw = $("pSweet");
  z.style.bottom = (ideal - zone) + "%";
  z.style.height = (zone * 2) + "%";
  sw.style.bottom = (ideal - zone * 0.5) + "%";
  sw.style.height = zone + "%";
  let powerLabel = "Sweet zone";
  if (s && s.super) powerLabel = "Halfcourt 10 pts power↑";
  else if (G.mode === "battle" && s && s.deep != null) powerLabel = "Color ball 5 pts power↑";
  else if (isDeep) powerLabel = "Deep ball power↑";
  $("pTag").textContent = currentWeather === "rain" ? "Rain boost · " + powerLabel : powerLabel;
...
function updDotsUI() {
  const s = curShot(),
    wrap = $("hudDots");
  if (!s) { wrap.innerHTML = ""; $("hudRackName").textContent = "Done!"; return; }
  if (G.mode === "battle") {
    const sp = BATTLE_SPOTS[G.battleSpot || 0];
    const st = battleSpotStatus(G.battleSpot || 0);
    $("hudRackName").textContent = (sp.super ? "🔥 " : "🎯 ") + sp.n + " (" + sp.val + " pts · " + st.label + ")";
  ...
  if (G.mode === "rackrush") {
    const rush = G.rush,
      pos = (rush ? rush.shotNo : 0) % 5,
      next = pos + 1;
    if (isRackRushSpeed(rush)) {
      $("hudRackName").textContent = next === 5 ? "Color ball up · 4 pts" : "Speed 100 · next ball 3 pts";
    } else {
      $("hudRackName").textContent = (next === 5 ? "Money ball up · 3 pts" : "Nonstop feed · next ball 2 pts") +
        (G.timer <= 10 ? " + FINAL BONUS" : "");
    }
    let h = "";
    for (let b = 0; b < 5; b++) h += '<i class="' + (b === 4 ? "gold" : "") + (b < pos ? " used" : "") + '"></i>';
    wrap.innerHTML = h;
    return;
  }
  if (s.deep != null) {
    $("hudRackName").textContent = "⚡ MINE-DEW deep three (3 pts)";
    wrap.innerHTML = '<i class="deep"></i>';
    return;
  }
  $("hudRackName").textContent = RACKS[s.rack].n + (s.rack === G.moneyRack ? " 💰 All-money rack" : "");
  let h = "";
  for (let b = 0; b < 5; b++) {
...
function startCharge() {
  ...
  if (!G.practice && G.seq && G.shotIdx === G.seq.length - 1 && G.mode === "contest") playAudioEvent("final_shot");
  else if (Math.random() < 0.28) playerEffort("load");
  return true;
}
...
function releaseShot(power, shot) {
  ...
  pBall.visible = false;
  const dirH = HOOP.clone().sub(p0);
  dirH.y = 0;
  const dist = dirH.length();
  dirH.normalize();
  const perp = V3(dirH.z, 0, -dirH.x);
  let depth = 0,
    lat = 0;
  if (outcome === "swish") { depth = rnd(-0.03, 0.03);
    lat = clamp(latErr * 0.5, -0.06, 0.06) + rnd(-0.02, 0.02); } else if (outcome === "rattle" || outcome === "rattleout") { depth = 0.17 * Math.sign(err || 1);
    lat = clamp(latErr, -0.12, 0.12) + rnd(-0.05, 0.05); } else if (outcome === "bank") { depth = 0.55;
    lat = clamp(latErr * 0.5, -0.1, 0.1); } else if (outcome === "rimout") { depth = 0.27 * Math.sign(err || 1);
    lat = clamp(latErr * 1.2, -0.2, 0.2) + rnd(-0.07, 0.07); } else { depth = err > 0 ? 0.58 : clamp(err * 0.055, -1.5, -0.55);
    lat = clamp(latErr * 2, -0.9, 0.9) + rnd(-0.2, 0.2); }
  let T;
  ...
  blob.position.set(p0.x, 0.02, p0.z);
  scene.add(blob);
  ...
  const B = {
    ...
    hot,
    startPos: p0.clone(),
    shooterPos: P.pos.clone(),
    shooterFace: P.face,
    superChanceId: shot.superChanceId || 0
  };
  B.resultClutch = noteResultAttempt(shot);
  balls.push(B);
  G.lastErr = err;
  // stats
  ...
  if (isRush) {
    ...
    G.shotIdx++;
    G.canShoot = false;
    const cfg = RACK_RUSH_LEVELS[rush.level],
      expectedCharge = shotIdeal(shot) / Math.max(1, playerChargeRate());
  ...
  const nxt = curShot();
  if (nxt) {
    if (G.mode === "contest" && shot && nxt.rack !== shot.rack && shot.rack !== null) {
      if (nxt.rack === G.moneyRack) playAudioEvent("contest_moneyrack");
      else if (nxt.rack === 4) playAudioEvent("contest_finalrack");
    }
    const samePos = isSameShotSpot(nxt, shot);
    afterPlayerLands(samePos ? 260 : 200, () => {
      if (G.state !== "round" && G.state !== "tiebreak") return;
      if (samePos) { readyBall(); } else { walkTo(nxt, () => readyBall()); }
    });
  }
}
...
function madeBall(b) {
  ...
  if (G.practice) {
    ...
    madeShotSound(b);
    cheerSound(false);
    G.cheer = Math.min(1, G.cheer + 0.4);
    if (navigator.vibrate) navigator.vibrate(18);
    popScore("✔ Made", "#7CFC6B");
    toast(CHEERS[(Math.random() * CHEERS.length) | 0]);
    return;
  }
  noteResultMake(b);
  if (G.mode === "rackrush") {
    ...
    const rush = G.rush,
      cfg = RACK_RUSH_LEVELS[rush.level],
      target = isRackRushSpeed(rush) ? RACK_RUSH_SPEED_TARGET : rackRushTarget(rush.level);
    const prevTotal = rush.total;
    rush.levelScore += b.val;
    rush.total += b.val;
    rush.makes++;
    rush.levelMakes++;
    if (isRackRushSpeed(rush)) {
      if (prevTotal < 25 && rush.total >= 25) playAudioEvent("speed100_25");
      else if (prevTotal < 50 && rush.total >= 50) playAudioEvent("speed100_50");
      else if (prevTotal < 75 && rush.total >= 75) playAudioEvent("speed100_75");
      else if (prevTotal < 90 && rush.total >= 90) playAudioEvent("speed100_90");
    }
    G.score = rush.total;
    G.streak++;
    G.missRun = 0;
    rush.bestStreak = Math.max(rush.bestStreak, G.streak);
    bloomOnScore(b.val);
    netPulse = 1;
    madeShotSound(b);
    const big = b.money || b.bonus || G.streak >= 5;
    cheerSound(big);
    G.cheer = Math.min(1, G.cheer + (big ? .8 : .45));
    ...
    if (big && !momentumSpoke) {
      let msg = "";
      if (b.money) msg += "Money ball! ";
      if (b.bonus) msg += "Final 10s bonus · ";
      msg += CHEERS[(Math.random() * CHEERS.length) | 0];
      toast(msg, b.bonus ? "#ff8d7a" : "#ffd23f");
    }
    if (G.streak >= 3) { $("hudStreak").style.display = "block";
      $("hudStreak").textContent = "🔥 x" + G.streak; }
    if (ending) { updateRackRushHUD();
      updJumbo();
      finishRackRushSpeed(); return; }
    if (!cfg.final && !rush.levelCleared && rush.levelScore >= target) { rush.levelCleared = true;
      rackRushClearFlash(); }
    updateRackRushHUD();
    updJumbo();
    return;
  ...
  if (G.mode === "battle") {
    ...
    cheerSound(big);
    G.cheer = Math.min(1, G.cheer + (big ? 1 : 0.6));
    if (navigator.vibrate) navigator.vibrate(big ? [20, 30, 20] : 18);
    let color = "#7CFC6B";
    let label = "+" + b.val;
    if (b.super) { color = "#ffd23f";
      label += " (Halfcourt 10)"; } else if (special) { color = "#54e05a";
      label += " (Color 5)"; }
    popScore(label, color);
    const ending = G.score >= BATTLE_TARGET;
    if (ending) beginFinalAudioWindow();
    const momentumSpoke = !ending && triggerMakeRunVoice();
    if (!momentumSpoke) {
      let msg = "";
      if (b.super) msg += "Halfcourt 10! ";
      else if (special) msg += "Color ball 5! ";
      msg += CHEERS[(Math.random() * CHEERS.length) | 0];
      if (G.streak >= 4) msg += " x" + G.streak;
      toast(msg, b.super ? "#ffd23f" : (special ? "#54e05a" : "#7CFC6B"));
    }
    if (!ending && b.super) gameDjSay("From way downtown, 10 points!", "special", 2.2);
    else if (!ending && special) gameDjSay("Color spot, five points!", "special", 2.0);
    if (G.streak >= 3) { $("hudStreak").style.display = "block";
      $("hudStreak").textContent = "🔥 x" + G.streak; }
    if (!ending) { battleCheckSuperMilestones();
  ...
  // Lead detection → queue cutaway
  if (G.score >= CUTAWAY_MIN_SCORE) {
    G.posted.forEach(p => {
      if (!p.cut && prev <= p.score && G.score > p.score) { p.cut = true;
        G.cutQ.push(p); }
    });
  }
  updTargetUI();
  ...
  let color = "#7CFC6B";
  let label = "+" + b.val;
  if (b.deep) color = "#54e05a";
  else if (b.money) color = "#ffd23f";
  popScore(label, color);
  const endingRound = G.seq && G.shotIdx >= G.seq.length;
  if (endingRound) { beginFinalAudioWindow(); if (G.mode === "contest" && typeof extPlay === "function") extPlay("crowdFinalMake"); }
  const momentumSpoke = !endingRound && triggerMakeRunVoice();
  if (!momentumSpoke) {
    let msg = "";
    if (b.deep) msg += "⚡ Deep three! ";
    msg += CHEERS[(Math.random() * CHEERS.length) | 0];
    if (G.streak >= 4) msg += " 🔥x" + G.streak;
    toast(msg);
  }
  if (!endingRound && b.deep) gameDjSay("Deep three!", "special", 2.0);
  if (G.streak >= 3) { $("hudStreak").style.display = "block";
    $("hudStreak").textContent = "🔥 x" + G.streak; }
  updJumbo();
}
...
function missBall() {
  ...
  if (G.practice) {
    const e = G.lastErr || 0;
    toast(e > 0 ? "Overcooked · released past the peak" : "Too early · wait for the peak", "#ff8d7a");
    return;
  }
  if (G.mode === "battle") {
    if (G.state !== "battle" || G.battleOver) return;
  ...
  G.missRun = (G.missRun || 0) + 1;
  noteResultMissRun();
  if (G.mode === "contest" && G.seq && G.shotIdx >= G.seq.length) {
    if (typeof extPlay === "function") extPlay("crowdFinalMiss");
  } else {
    if (Math.random() < 0.45) aww();
  }
  if (triggerMissRunVoice()) return;
  // Consecutive bricks → opponent trash talk
    ...
    if (b.phase === "fly") {
      ...
      if (b.t >= b.tf) {
        ...
        if (b.outcome === "swish") {
          ...
          playerRimHaptic(b);
          b.rin = b.outcome === "rattle";
          playRimImpactSound(b, b.rin);
          b.phase = "rattle";
          b.rt = 0;
          ...
          playerRimHaptic(b);
          playRimImpactSound(b, false);
          if (b.opp) triggerStreetCrowdReaction("oppMiss", 0);
          else if (b.silent) { if (typeof announceAIShowResult === "function") announceAIShowResult(b, false); } else missBall();
          b.phase = "free";
